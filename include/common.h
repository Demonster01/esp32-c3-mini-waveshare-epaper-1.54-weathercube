...
// #########################################################################################
//  Problems with stucturing JSON decodes, see here:
...
bool DecodeWeather(WiFiClient &json, String Type) {
  ...
  if (Type == "weather") {
    // All Serial.println statements are for diagnostic purposes and not
    // required, remove if not needed
    ...
    WxConditions[0].Forecast0 =
        root["weather"][0]["description"].as<const char *>();
    // Serial.println("For0: " + String(WxConditions[0].Forecast0));
    WxConditions[0].Forecast1 =
        root["weather"][1]["description"].as<const char *>();
    // Serial.println("For1: " + String(WxConditions[0].Forecast1));
    WxConditions[0].Forecast2 =
        root["weather"][2]["description"].as<const char *>();
    ...
    // Serial.println("CCov: " + String(WxConditions[0].Cloudcover)); // in % of
    // cloud cover
    WxConditions[0].Visibility = root["visibility"].as<int>();
    // Serial.println("Visi: " + String(WxConditions[0].Visibility)); // in
    // metres
  ...
  if (Type == "forecast") {
    ...
    for (byte r = 0; r < max_readings; r++) {
      ...
      WxForecast[r].Forecast0 =
          list[r]["weather"][0]["main"].as<const char *>();
      // Serial.println("For0: " + String(WxForecast[r].Forecast0));
      WxForecast[r].Forecast1 =
          list[r]["weather"][1]["main"].as<const char *>();
      // Serial.println("For1: " + String(WxForecast[r].Forecast1));
      WxForecast[r].Forecast2 =
          list[r]["weather"][2]["main"].as<const char *>();
    ...
    float pressure_trend =
        WxForecast[2].Pressure -
        WxForecast[0].Pressure; // Measure pressure slope between ~now and later
    pressure_trend = ((int)(pressure_trend * 10)) /
                     10.0; // Remove any small variations less than 0.1
...
bool obtain_wx_data(WiFiClient &client, const String &RequestType) {
  ...
  // String uri = "/data/3.0/onecall?lat=" + LAT + "&lon=" + LON + "&appid=" +
  // apikey + "&mode=json&units=" + units + "&lang=" + Language;
  String uri = "/data/2.5/" + RequestType + "?lat=" + LAT + "&lon=" + LON +
               "&appid=" + apikey + "&mode=json&units=" + units +
               "&lang=" + Language;
